...
class Fr3PegInsertEnv(DirectRLEnv):
    cfg: Fr3PegInsertEnvCfg

    def __init__(self, cfg: Fr3PegInsertEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        utils.set_body_inertias(self._robot, self.scene.num_envs)
        self._init_tensors()
    ...
    def _set_default_dynamics_parameters(self):
        ...
        # Set frictions.
        utils.set_friction(self._held_asset, self.cfg_task.held_asset_cfg.friction, self.scene.num_envs)
        utils.set_friction(self._fixed_asset, self.cfg_task.fixed_asset_cfg.friction, self.scene.num_envs)
        utils.set_friction(self._robot, self.cfg_task.robot_cfg.friction, self.scene.num_envs)
    ...
    def _get_dones(self):
        ...
        For reset logic, it is important that all environments
    ...
    def _get_curr_successes(self, success_threshold):
        ...
        target_held_base_pos, target_held_base_quat = utils.get_target_held_base_pose(
            self.fixed_pos,
            self.fixed_quat,
            self.num_envs,
            self.device,
    ...
    def _get_rew_dict(self, curr_successes):
        ...
        offsets = utils.get_keypoint_offsets(self.cfg_task.num_keypoints, self.device)
        ...
        rew_dict = {
            "kp_baseline": utils.squashing_fn(keypoint_dist, a0, b0),
            "kp_coarse": utils.squashing_fn(keypoint_dist, a1, b1),
    ...
    def set_pos_inverse_kinematics(
        ...
        while ik_time < 0.25:
            ...
            pos_error, axis_angle_error = control.get_pose_error(
            ...
            # Solve DLS problem.
            delta_dof_pos = control.get_delta_dof_pos(
    ...
    def randomize_initial_state(self, env_ids):
        ...
        self.task_deriv_gains = utils.get_deriv_gains(reset_task_prop_gains, self.cfg.ctrl.reset_rot_deriv_scale)
        ...
        self.task_deriv_gains = utils.get_deriv_gains(self.default_gains)
